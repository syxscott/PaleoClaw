...
```bash
cat ~/.paleoclaw/paleoclaw.json | jq '.gateway.bind'
```
...
1. Check server bind: `cat ~/.paleoclaw/paleoclaw.json | jq '.gateway.bind'`
...
Node is offline. Use `paleoclaw nodes list` to find online nodes.
