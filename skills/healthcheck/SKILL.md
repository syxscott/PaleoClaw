...
# paleoclaw Host Hardening

## Overview

Assess and harden the host running paleoclaw, then align it to a user-defined risk tolerance without breaking access. Use paleoclaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim paleoclaw changes the host firewall, SSH, or OS updates; it does not.
...
5. paleoclaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if paleoclaw can access sensitive data.
...
### 2) Run paleoclaw security audits (read-only)

As part of the default read-only checks, run `paleoclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `paleoclaw security audit` (faster, non-probing)
2. `paleoclaw security audit --json` (structured output)

Offer to apply paleoclaw safe defaults (numbered):

1. `paleoclaw security audit --fix`

Be explicit that `--fix` only tightens paleoclaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check paleoclaw version/update status (read-only)

As part of the default read-only checks, run `paleoclaw update status`.
...
- Credential hygiene notes (location of paleoclaw creds, prefer disk encryption)
...
- paleoclaw security audit (re-run)
...
After paleoclaw install or first hardening pass, run at least one baseline audit and version check:

- `paleoclaw security audit`
- `paleoclaw security audit --deep`
- `paleoclaw update status`

Ongoing monitoring is recommended. Use the paleoclaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `paleoclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `paleoclaw update status`
...
Before creating, `paleoclaw cron list` and match on exact `name`. If found, `paleoclaw cron edit <id> ...`.
If not found, `paleoclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `paleoclaw update status` (preferred for source checkouts and channels)
2. `npm view paleoclaw version` (published npm version)

## paleoclaw command accuracy

Use only supported commands and flags:

- `paleoclaw security audit [--deep] [--fix] [--json]`
- `paleoclaw status` / `paleoclaw status --deep`
- `paleoclaw health --json`
- `paleoclaw update status`
- `paleoclaw cron add|list|runs|run`

Do not invent CLI flags or imply paleoclaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by paleoclaw compaction:
