 ...
 */
export async function exportSpecimen(result: SpecimenResult, config: ExportConfig): Promise<void> {
 ...
 * @param config - Export configuration
 */
export async function exportBatch(batch: BatchResult, config: ExportConfig): Promise<void> {
  await mkdir(dirname(config.outputPath), { recursive: true });
 ...
 */
async function exportToTPS(result: SpecimenResult, config: ExportConfig): Promise<void> {
  const lines: string[] = [];
 ...
 * 导出批量结果到单个 TPS 文件
 */
async function exportBatchToTPS(batch: BatchResult, config: ExportConfig): Promise<void> {
  const lines: string[] = [];
 ...
 */
async function exportToCSV(result: SpecimenResult, config: ExportConfig): Promise<void> {
  const lines: string[] = [];
  ...
  // Data rows
  for (const lm of result.landmarks) {
    const row = [result.specimenId, lm.id, lm.x.toFixed(6), lm.y.toFixed(6), lm.type];
    if (config.includeNames) {
      row.push(lm.name || "");
    }
    lines.push(row.join(","));
 ...
 * 导出批量结果到 CSV
 */
async function exportBatchToCSV(batch: BatchResult, config: ExportConfig): Promise<void> {
  const lines: string[] = [];
 ...
 */
async function exportToExcel(result: SpecimenResult, config: ExportConfig): Promise<void> {
 ...
 * 导出批量结果到 Excel
 */
async function exportBatchToExcel(batch: BatchResult, config: ExportConfig): Promise<void> {
  // For now, export CSV-compatible content to the requested Excel path.
  const csvConfig = { ...config, outputPath: config.outputPath };
 ...
 */
async function exportToJSON(result: SpecimenResult, config: ExportConfig): Promise<void> {
  const data = {
    ...result,
    exportInfo: {
      format: "json",
 ...
 * 导出批量结果到 JSON
 */
async function exportBatchToJSON(batch: BatchResult, config: ExportConfig): Promise<void> {
  const data = {
    ...batch,
    exportInfo: {
      format: "json",
 ...
 */
export function createExportConfig(format: ExportFormat, outputPath: string): ExportConfig {
  return {
    format,
    outputPath,
    includeNames: true,
