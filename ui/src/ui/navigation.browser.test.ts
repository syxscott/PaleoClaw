...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/paleoclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/paleoclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/paleoclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = "/paleoclaw";
    const app = mountApp("/paleoclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/paleoclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/paleoclaw/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "paleoclaw.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/paleoclaw" }),
    );
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
