...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /paleoclaw/ ");

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/paleoclaw"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/paleoclaw/chat",
    });

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/paleoclaw"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/paleoclaw",
    ...
    expect(JSON.parse(localStorage.getItem("paleoclaw.control.settings.v1") ?? "{}")).toEqual({
      gatewayUrl: "wss://gateway.example:8443/paleoclaw",
      sessionKey: "agent",
