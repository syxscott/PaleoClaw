...
metadata: { "paleoclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == paleoclaw `sessions_spawn`
- **File I/O** == paleoclaw `read`/`write`
- **Remote fetch** == paleoclaw `web_fetch` (or `exec` with curl when POST is required)
