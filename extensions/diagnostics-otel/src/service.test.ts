...
vi.mock("paleoclaw/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("paleoclaw/plugin-sdk/diagnostics-otel")>(
    "paleoclaw/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { OpenClawPluginServiceContext } from "paleoclaw/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "paleoclaw/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/paleoclaw-diagnostics-otel-test";
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("paleoclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("paleoclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("paleoclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("paleoclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("paleoclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("paleoclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("paleoclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("paleoclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("paleoclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("paleoclaw.webhook.processed");
    expect(spanNames).toContain("paleoclaw.message.processed");
    expect(spanNames).toContain("paleoclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["paleoclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("paleoclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "paleoclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["paleoclaw.reason"]).toBe("string");
    expect(String(attrs?.["paleoclaw.reason"])).not.toContain(
