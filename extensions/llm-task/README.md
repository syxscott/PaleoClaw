...
`paleoclaw.invoke --each`) without adding new paleoclaw code per workflow.
...
This extension depends on paleoclaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** paleoclaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.paleoclaw/extensions` as a standalone plugin directory.
