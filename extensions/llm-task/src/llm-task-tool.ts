...
import Ajv from "ajv";
import { resolvePreferredOpenClawTmpDir } from "paleoclaw/plugin-sdk/llm-task";
// NOTE: This extension is intended to be bundled with paleoclaw.
// When running from source (tests/dev), paleoclaw internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { OpenClawPluginApi } from "paleoclaw/plugin-sdk/llm-task";
...
export function createLlmTaskTool(api: OpenClawPluginApi) {
  return {
    ...
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via paleoclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredOpenClawTmpDir(), "paleoclaw-llm-task-"),
        );
