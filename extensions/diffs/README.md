...
Set plugin-wide defaults in `~/.paleoclaw/paleoclaw.json`:
...
Before:
paleoclaw supports plugins.

After:
paleoclaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/paleoclaw-diffs`).
