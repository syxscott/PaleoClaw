...
Tip: `paleoclaw config` without a subcommand opens the same wizard. Use
`paleoclaw config get|set|unset` for non-interactive edits.
...
```bash
paleoclaw configure
paleoclaw configure --section model --section channels
```
