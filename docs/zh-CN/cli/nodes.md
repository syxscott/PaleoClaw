...
read_when:
  ...
  - 你需要批准请求或调用节点命令
summary: "`paleoclaw nodes` 的 CLI 参考（列表/状态/批准/调用，摄像头/画布/屏幕）"
title: nodes
x-i18n:
...
paleoclaw nodes list --connected
paleoclaw nodes list --last-connected 24h
paleoclaw nodes pending
paleoclaw nodes approve <requestId>
paleoclaw nodes status
paleoclaw nodes status --connected
paleoclaw nodes status --last-connected 24h
```
...
```bash
paleoclaw nodes invoke --node <id|name|ip> --command <command> --params <json>
paleoclaw nodes run --node <id|name|ip> <command...>
paleoclaw nodes run --raw "git status"
paleoclaw nodes run --agent main --node <id|name|ip> --raw "git status"
```
