...
你可以在 `~/.paleoclaw/paleoclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.paleoclaw/paleoclaw.json` 的 `logging` 下。
...
- paleoclaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `paleoclaw plugins enable diagnostics-otel` 启用插件。
...
- `paleoclaw.tokens`（计数器，属性：`paleoclaw.token`、`paleoclaw.channel`、`paleoclaw.provider`、`paleoclaw.model`）
- `paleoclaw.cost.usd`（计数器，属性：`paleoclaw.channel`、`paleoclaw.provider`、`paleoclaw.model`）
- `paleoclaw.run.duration_ms`（直方图，属性：`paleoclaw.channel`、`paleoclaw.provider`、`paleoclaw.model`）
- `paleoclaw.context.tokens`（直方图，属性：`paleoclaw.context`、`paleoclaw.channel`、`paleoclaw.provider`、`paleoclaw.model`）

消息流：

- `paleoclaw.webhook.received`（计数器，属性：`paleoclaw.channel`、`paleoclaw.webhook`）
- `paleoclaw.webhook.error`（计数器，属性：`paleoclaw.channel`、`paleoclaw.webhook`）
- `paleoclaw.webhook.duration_ms`（直方图，属性：`paleoclaw.channel`、`paleoclaw.webhook`）
- `paleoclaw.message.queued`（计数器，属性：`paleoclaw.channel`、`paleoclaw.source`）
- `paleoclaw.message.processed`（计数器，属性：`paleoclaw.channel`、`paleoclaw.outcome`）
- `paleoclaw.message.duration_ms`（直方图，属性：`paleoclaw.channel`、`paleoclaw.outcome`）

队列 + 会话：

- `paleoclaw.queue.lane.enqueue`（计数器，属性：`paleoclaw.lane`）
- `paleoclaw.queue.lane.dequeue`（计数器，属性：`paleoclaw.lane`）
- `paleoclaw.queue.depth`（直方图，属性：`paleoclaw.lane` 或 `paleoclaw.channel=heartbeat`）
- `paleoclaw.queue.wait_ms`（直方图，属性：`paleoclaw.lane`）
- `paleoclaw.session.state`（计数器，属性：`paleoclaw.state`、`paleoclaw.reason`）
- `paleoclaw.session.stuck`（计数器，属性：`paleoclaw.state`）
- `paleoclaw.session.stuck_age_ms`（直方图，属性：`paleoclaw.state`）
- `paleoclaw.run.attempt`（计数器，属性：`paleoclaw.attempt`）
...
- `paleoclaw.webhook.error`
  - `paleoclaw.channel`、`paleoclaw.webhook`、`paleoclaw.chatId`、`paleoclaw.error`
- `paleoclaw.message.processed`
  - `paleoclaw.channel`、`paleoclaw.outcome`、`paleoclaw.chatId`、`paleoclaw.messageId`、`paleoclaw.sessionKey`、`paleoclaw.sessionId`、`paleoclaw.reason`
- `paleoclaw.session.stuck`
  - `paleoclaw.state`、`paleoclaw.ageMs`、`paleoclaw.queueDepth`、`paleoclaw.sessionKey`、`paleoclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `paleoclaw doctor`。
