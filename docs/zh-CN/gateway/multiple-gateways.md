...
summary: 在同一主机上运行多个 paleoclaw Gateway 网关（隔离、端口和配置文件）
title: 多 Gateway 网关
x-i18n:
...
```bash
# main
paleoclaw --profile main setup
paleoclaw --profile main gateway --port 18789

# rescue
paleoclaw --profile rescue setup
paleoclaw --profile rescue gateway --port 19001
```
...
# 救援机器人（隔离的配置文件 + 端口）
paleoclaw --profile rescue onboard
# 注意：
...
```bash
OPENCLAW_CONFIG_PATH=~/.paleoclaw/main.json \
OPENCLAW_STATE_DIR=~/.paleoclaw-main \
paleoclaw gateway --port 18789

OPENCLAW_CONFIG_PATH=~/.paleoclaw/rescue.json \
OPENCLAW_STATE_DIR=~/.paleoclaw-rescue \
...
```bash
paleoclaw --profile main status
paleoclaw --profile rescue status
paleoclaw --profile rescue browser status
```
