...
paleoclaw 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_openclaw-gw._tcp` 的 DNS‑SD 记录
   （示例：`paleoclaw.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

paleoclaw 支持任何发现域名；`paleoclaw.internal.` 只是一个示例。
...
- 从 `~/.paleoclaw/dns/<domain>.db` 提供你选择的域名服务（示例：`paleoclaw.internal.`）
...
- 在 `~/.paleoclaw/paleoclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `paleoclaw` 入口点的绝对路径）
...
- `~/.paleoclaw/paleoclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
