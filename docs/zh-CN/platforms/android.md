...
- 你可以在 Gateway 网关机器上运行 CLI（`paleoclaw`）（或通过 SSH）。
...
- 在 Gateway 网关主机的 `~/.paleoclaw/paleoclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `paleoclaw.internal.`）并发布 `_openclaw-gw._tcp` 记录。
...
1. 在 Gateway 网关主机上创建 `~/.paleoclaw/workspace/canvas/index.html`。

2. 将节点导航到它（局域网）：

```bash
paleoclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```
