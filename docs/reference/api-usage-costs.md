...
paleoclaw features that can generate provider usage or paid API calls.
...
- `paleoclaw status --usage` and `paleoclaw channels list` show provider **usage windows**
...
paleoclaw can pick up credentials from:
...
- `paleoclaw status --usage`
- `paleoclaw models status --json`
...
`paleoclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
