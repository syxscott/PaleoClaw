...
- With Anthropic API-key auth profiles, paleoclaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, paleoclaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
paleoclaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.paleoclaw/logs/cache-trace.jsonl" # optional
