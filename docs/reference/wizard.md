...
This is the full reference for the `paleoclaw onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.paleoclaw/credentials/oauth.json`; auth profiles live in `~/.paleoclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.paleoclaw/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.paleoclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `paleoclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `paleoclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `paleoclaw health`.
    - Tip: `paleoclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
- Stores it under `~/.paleoclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.paleoclaw/paleoclaw.json`:
...
`paleoclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.paleoclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.paleoclaw/agents/<agentId>/sessions/`.
