...
paleoclaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- paleoclaw owns lifecycle when `channels.signal.autoStart=true`.
...
- paleoclaw spawns `imsg rpc` as a child process (legacy iMessage integration).
