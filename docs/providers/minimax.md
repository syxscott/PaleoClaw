...
```bash
paleoclaw plugins enable minimax-portal-auth  # skip if already loaded.
paleoclaw gateway restart  # restart if gateway is already running
paleoclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/paleoclaw/paleoclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `paleoclaw.json`:
...
- Use `paleoclaw models list` and `paleoclaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `paleoclaw configure` and selecting **MiniMax M2.5**, or
