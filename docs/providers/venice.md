...
```bash
paleoclaw onboard --auth-choice venice-api-key
```
...
```bash
paleoclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, paleoclaw shows all available Venice models. Pick based on your needs:
...
```bash
paleoclaw models set venice/kimi-k2-5
paleoclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
paleoclaw models list | grep venice
```

## Configure via `paleoclaw configure`

1. Run `paleoclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
paleoclaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
paleoclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
paleoclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
paleoclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
paleoclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
paleoclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `paleoclaw models list` to see currently available models. Some models may be temporarily offline.
