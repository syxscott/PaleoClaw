...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like paleoclaw.
...
```bash
paleoclaw onboard --auth-choice openai-api-key
# or non-interactive
paleoclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. paleoclaw forwards both through the `openai/*` Responses path.
...
paleoclaw onboard --auth-choice openai-codex

# Or run OAuth directly
paleoclaw models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. paleoclaw
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

paleoclaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), paleoclaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. paleoclaw enables it by default for
...
paleoclaw, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), paleoclaw now auto-enables OpenAI server-side
