...
paleoclaw can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, paleoclaw can automatically discover Bedrock
...
When running paleoclaw on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, paleoclaw's credential detection currently only checks for environment
...
- paleoclaw surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
