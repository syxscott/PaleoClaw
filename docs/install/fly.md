...
read_when:
  - Deploying paleoclaw on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** paleoclaw Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
```toml
app = "my-paleoclaw"  # Your app name
primary_region = "iad"
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `paleoclaw.json` where they could be accidentally exposed or logged.
...
**Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/paleoclaw.json`.
...
Or visit `https://my-paleoclaw.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/paleoclaw.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/paleoclaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/paleoclaw.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/paleoclaw.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/paleoclaw.json"
```
...
```bash
# List current IPs
fly ips list -a my-paleoclaw

# Release public IPs
fly ips release <public-ipv4> -a my-paleoclaw
fly ips release <public-ipv6> -a my-paleoclaw
...
fly ips allocate-v6 --private -a my-paleoclaw
```
...
fly proxy 3000:3000 -a my-paleoclaw
...
```bash
fly ssh console -a my-paleoclaw
```
