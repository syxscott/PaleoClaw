...
- **Yes**: you want an isolated, throwaway gateway environment or to run paleoclaw on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full paleoclaw in Docker)
...
- `OPENCLAW_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/paleoclaw/paleoclaw:latest`)
...
- Need the URL again? Run `docker compose run --rm paleoclaw-cli dashboard --no-open`.
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `paleoclaw-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
...
```bash
docker compose run -T --rm paleoclaw-cli gateway probe
docker compose run -T --rm paleoclaw-cli devices list --json
```
...
`paleoclaw-cli` uses `network_mode: "service:paleoclaw-gateway"` so CLI commands can
...
network path instead of the bundled `paleoclaw-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `paleoclaw-cli`.
...
- [GitHub Container Registry package](https://github.com/paleoclaw/paleoclaw/pkgs/container/paleoclaw)

Use image name `ghcr.io/paleoclaw/paleoclaw` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/paleoclaw/paleoclaw`
- `org.opencontainers.image.url=https://paleoclaw.ai`
- `org.opencontainers.image.documentation=https://docs.paleoclaw.ai/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=paleoclaw`
- `org.opencontainers.image.description=paleoclaw gateway and CLI runtime container image`
...
```bash
export OPENCLAW_IMAGE="ghcr.io/paleoclaw/paleoclaw:latest"
./docker-setup.sh
```

The script detects that `OPENCLAW_IMAGE` is not the default `paleoclaw:local` and
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/paleoclaw/paleoclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/paleoclaw/paleoclaw/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t paleoclaw:local -f Dockerfile .
docker compose run --rm paleoclaw-cli onboard
docker compose up -d paleoclaw-gateway
```
...
```bash
docker compose run --rm paleoclaw-cli dashboard --no-open
docker compose run --rm paleoclaw-cli devices list
docker compose run --rm paleoclaw-cli devices approve <requestId>
```
...
`paleoclaw-gateway` and `paleoclaw-cli` by generating `docker-compose.extra.yml`.
...
`/home/node/.paleoclaw`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/paleoclaw-config /path/to/paleoclaw-workspace
```
...
```bash
docker compose run --rm paleoclaw-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm paleoclaw-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm paleoclaw-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if paleoclaw is still
...
```bash
docker compose exec paleoclaw-gateway node dist/index.js health --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
```bash
docker compose run --rm paleoclaw-cli config set gateway.mode local
docker compose run --rm paleoclaw-cli config set gateway.bind lan
docker compose run --rm paleoclaw-cli devices list --url ws://127.0.0.1:18789
```
...
- The gateway container is the source of truth for sessions (`~/.paleoclaw/agents/<agentId>/sessions/`).

### Storage model

- **Persistent host data:** Docker Compose bind-mounts `OPENCLAW_CONFIG_DIR` to `/home/node/.paleoclaw` and `OPENCLAW_WORKSPACE_DIR` to `/home/node/.paleoclaw/workspace`, so those paths survive container replacement.
...
- **Disk growth hotspots:** watch `media/`, `agents/<agentId>/sessions/sessions.json`, transcript JSONL files, `cron/runs/*.jsonl`, and rolling file logs under `/tmp/paleoclaw/` (or your configured `logging.file`). If you also run the macOS app outside Docker, its service logs are separate again: `~/.paleoclaw/logs/gateway.log`, `~/.paleoclaw/logs/gateway.err.log`, and `/tmp/paleoclaw/paleoclaw-gateway.log`.
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.paleoclaw/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  paleoclaw auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `paleoclaw sandbox recreate ...` command.
...
This builds `paleoclaw-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `paleoclaw-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "paleoclaw-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `paleoclaw-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`paleoclaw-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; paleoclaw provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-paleoclaw-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-paleoclaw-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-paleoclaw-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/paleoclaw/paleoclaw/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: paleoclaw runs commands with `sh -lc` (login shell), which
