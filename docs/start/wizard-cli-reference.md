...
This page is the full reference for `paleoclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.paleoclaw/paleoclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `paleoclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `paleoclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `paleoclaw health`.
    - `paleoclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.paleoclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.paleoclaw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.paleoclaw/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`paleoclaw onboard`](/cli/onboard)
