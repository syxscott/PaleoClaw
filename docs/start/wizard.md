...
The onboarding wizard is the **recommended** way to set up paleoclaw on macOS,
...
`paleoclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`paleoclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.paleoclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `paleoclaw doctor` first.
</Note>
...
Use `paleoclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.paleoclaw/workspace-<agentId>`.
...
- CLI command reference: [`paleoclaw onboard`](/cli/onboard)
