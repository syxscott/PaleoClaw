...
```bash
paleoclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `paleoclaw tui --deliver`
...
- Check the Gateway logs: `paleoclaw logs --follow`.
- Confirm the agent can run: `paleoclaw status` and `paleoclaw models status`.
...
- No agents in picker: check `paleoclaw agents list` and your routing config.
