...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/paleoclaw`)
...
If the page fails to load, start the Gateway first: `paleoclaw gateway`.
...
you revoke it with `paleoclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.paleoclaw/paleoclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop paleoclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. paleoclaw
...
paleoclaw **blocks** Control UI connections without device identity.
