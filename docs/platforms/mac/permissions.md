...
- Same path: run the app from a fixed location (for paleoclaw, `dist/paleoclaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.paleoclaw.mac
sudo tccutil reset ScreenCapture ai.paleoclaw.mac
...
Workaround: move files into the paleoclaw workspace (`~/.paleoclaw/workspace`) if you want to avoid per-folder grants.
