...
service via the external `paleoclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.paleoclaw.gateway`
  (or `ai.paleoclaw.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.paleoclaw.*` is supported).
