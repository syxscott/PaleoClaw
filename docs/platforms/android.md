...
- You can run the CLI (`paleoclaw`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.paleoclaw/paleoclaw.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `paleoclaw.internal.`) on the gateway host and publish `_openclaw-gw._tcp` records.
...
1. Create `~/.paleoclaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
paleoclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__openclaw__/canvas/"}'
```
