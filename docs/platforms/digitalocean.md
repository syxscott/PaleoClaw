...
read_when:
  ...
  - Looking for cheap VPS hosting for paleoclaw
title: "DigitalOcean"
---

# paleoclaw on DigitalOcean

## Goal

Run a persistent paleoclaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status paleoclaw-gateway.service

# View logs
journalctl --user -u paleoclaw-gateway.service -f
```
...
```bash
paleoclaw channels login whatsapp
# Scan QR code
```
...
- `~/.paleoclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u paleoclaw --no-pager -n 50
```
