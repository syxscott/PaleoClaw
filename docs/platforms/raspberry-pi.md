...
read_when:
  ...
  - Running paleoclaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# paleoclaw on Raspberry Pi

## Goal

Run a persistent, always-on paleoclaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
paleoclaw config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/paleoclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/paleoclaw-compile-cache
mkdir -p /var/tmp/paleoclaw-compile-cache
...
If this Pi is mostly running paleoclaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit paleoclaw
```

```ini
[Service]
Environment=OPENCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/paleoclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep paleoclaw state/cache on SSD-backed storage to avoid SD-card
...
Most paleoclaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled paleoclaw

# Enable if not
sudo systemctl enable paleoclaw

# Start on boot
sudo systemctl start paleoclaw
```
...
```bash
# Check logs
journalctl -u paleoclaw --no-pager -n 100

# Common fix: rebuild
cd ~/paleoclaw  # if using hackable install
npm run build
sudo systemctl restart paleoclaw
```
