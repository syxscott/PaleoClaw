...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.paleoclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.paleoclaw/node.json` (node id + pairing token).
...
Path: `~/.paleoclaw/exec-approvals.json`
...
- Unix socket at `~/.paleoclaw/exec-approvals.sock` (0600).
