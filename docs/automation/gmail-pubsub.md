...
# Gmail Pub/Sub -> paleoclaw

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> paleoclaw webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- paleoclaw hooks enabled (see [Webhooks](/automation/webhook)).
...
under `~/.paleoclaw/hooks/transforms` (see [Webhooks](/automation/webhook)).

## Wizard (recommended)

Use the paleoclaw helper to wire everything together (installs deps on macOS via brew):

```bash
paleoclaw webhooks gmail setup \
...
- Writes `hooks.gmail` config for `paleoclaw webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, paleoclaw automatically sets
...
- `--hook-url` points to paleoclaw `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to paleoclaw.

Recommended: `paleoclaw webhooks gmail run` wraps the same flow and auto-renews the watch.
...
```bash
gog gmail watch status --account paleoclaw@gmail.com
gog gmail history --account paleoclaw@gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account paleoclaw@gmail.com
