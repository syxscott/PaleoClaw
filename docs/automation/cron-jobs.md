...
- Jobs persist under `~/.paleoclaw/cron/` so restarts don’t lose schedules.
...
paleoclaw cron list
paleoclaw cron run <job-id>
paleoclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.paleoclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `paleoclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, paleoclaw applies a
...
If `delivery` is omitted for isolated jobs, paleoclaw defaults to `announce`.
...
- CLI equivalents: `paleoclaw cron add --light-context ...` and `paleoclaw cron edit --light-context`.
...
- Job store: `~/.paleoclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.paleoclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, paleoclaw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
```json5
{
  cron: {
    ...
    store: "~/.paleoclaw/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, paleoclaw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `paleoclaw cron runs` and adjust retention before logs become large
...
paleoclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
paleoclaw cron edit <jobId> --agent ops
paleoclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
paleoclaw cron run <jobId>
paleoclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
paleoclaw cron edit <jobId> \
...
```bash
paleoclaw cron edit <jobId> --exact
```
...
```bash
paleoclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`paleoclaw system event`](/cli/system).
...
- paleoclaw applies exponential retry backoff for recurring jobs after consecutive errors:
