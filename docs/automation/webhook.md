...
read_when:
  ...
  - Wiring external systems into paleoclaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your paleoclaw config directory (typically `~/.paleoclaw/hooks/transforms`).
...
- `paleoclaw webhooks gmail setup` writes `hooks.gmail` config for `paleoclaw webhooks gmail run`.
