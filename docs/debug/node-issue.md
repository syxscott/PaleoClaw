...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in paleoclaw
...
Running paleoclaw via Node with `tsx` fails at startup with:

```
[paleoclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `paleoclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch paleoclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node paleoclaw.mjs status` works on Node 25.
