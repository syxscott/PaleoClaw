...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.paleoclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.paleoclaw/workspace` as canonical human-readable memory.
...
```
~/.paleoclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in paleoclaw**, but keep a separable core library.

### Why integrate into paleoclaw?

- paleoclaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `paleoclaw memory recall "…" --k 25 --since 30d`
  - `paleoclaw memory reflect --since 7d`
...
Pragmatic take for `~/.paleoclaw/workspace`:
