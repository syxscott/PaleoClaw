...
This plan defines how paleoclaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- paleoclaw core owns ACP control plane concerns
...
paleoclaw should not reimplement ACP transport internals in core.
paleoclaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in paleoclaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under paleoclaw state dir
...
Core (must be in paleoclaw core):
...
- ACP event stream is normalized to paleoclaw reply chunks
...
For the first production backend (`extensions/acpx`), paleoclaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses paleoclaw `sessionKey`)
...
- paleoclaw consumes ndjson events from `acpx --format json --json-strict`
