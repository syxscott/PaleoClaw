...
- `/usage cost` in chats: local cost summary aggregated from paleoclaw session logs.
- CLI: `paleoclaw status --usage` prints a full per-provider breakdown.
- CLI: `paleoclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
