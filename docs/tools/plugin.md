...
A plugin is just a **small code module** that extends paleoclaw with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core paleoclaw yet (or you want to keep optional features out of your main
install).
...
```bash
paleoclaw plugins install @paleoclaw/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@paleoclaw/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@paleoclaw/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@paleoclaw/zalouser`
- [Matrix](/channels/matrix) — `@paleoclaw/matrix`
- [Nostr](/channels/nostr) — `@paleoclaw/nostr`
- [Zalo](/channels/zalo) — `@paleoclaw/zalo`
- [Microsoft Teams](/channels/msteams) — `@paleoclaw/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

paleoclaw plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `paleoclaw/plugin-sdk` import when
authoring plugins:

- `paleoclaw/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `paleoclaw/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `paleoclaw/plugin-sdk/telegram` for Telegram channel plugins.
- `paleoclaw/plugin-sdk/discord` for Discord channel plugins.
- `paleoclaw/plugin-sdk/slack` for Slack channel plugins.
- `paleoclaw/plugin-sdk/signal` for Signal channel plugins.
- `paleoclaw/plugin-sdk/imessage` for iMessage channel plugins.
- `paleoclaw/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `paleoclaw/plugin-sdk/line` for LINE channel plugins.
- `paleoclaw/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `paleoclaw/plugin-sdk/acpx`, `paleoclaw/plugin-sdk/bluebubbles`,
  `paleoclaw/plugin-sdk/copilot-proxy`, `paleoclaw/plugin-sdk/device-pair`,
  `paleoclaw/plugin-sdk/diagnostics-otel`, `paleoclaw/plugin-sdk/diffs`,
  `paleoclaw/plugin-sdk/feishu`,
  `paleoclaw/plugin-sdk/google-gemini-cli-auth`, `paleoclaw/plugin-sdk/googlechat`,
  `paleoclaw/plugin-sdk/irc`, `paleoclaw/plugin-sdk/llm-task`,
  `paleoclaw/plugin-sdk/lobster`, `paleoclaw/plugin-sdk/matrix`,
  `paleoclaw/plugin-sdk/mattermost`, `paleoclaw/plugin-sdk/memory-core`,
  `paleoclaw/plugin-sdk/memory-lancedb`,
  `paleoclaw/plugin-sdk/minimax-portal-auth`,
  `paleoclaw/plugin-sdk/nextcloud-talk`, `paleoclaw/plugin-sdk/nostr`,
  `paleoclaw/plugin-sdk/open-prose`, `paleoclaw/plugin-sdk/phone-control`,
  `paleoclaw/plugin-sdk/qwen-portal-auth`, `paleoclaw/plugin-sdk/synology-chat`,
  `paleoclaw/plugin-sdk/talk-voice`, `paleoclaw/plugin-sdk/test-utils`,
  `paleoclaw/plugin-sdk/thread-ownership`, `paleoclaw/plugin-sdk/tlon`,
  `paleoclaw/plugin-sdk/twitch`, `paleoclaw/plugin-sdk/voice-call`,
  `paleoclaw/plugin-sdk/zalo`, and `paleoclaw/plugin-sdk/zalouser`.

Compatibility note:

- `paleoclaw/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `paleoclaw status`, `paleoclaw status --all`,
  `paleoclaw channels status`, `paleoclaw channels resolve`, and doctor/config
...
- `<workspace>/.paleoclaw/extensions/*.ts`
- `<workspace>/.paleoclaw/extensions/*/index.ts`

3. Global extensions

- `~/.paleoclaw/extensions/*.ts`
- `~/.paleoclaw/extensions/*/index.ts`

4. Bundled extensions (shipped with paleoclaw, mostly disabled by default)

- `<paleoclaw>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `paleoclaw plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, paleoclaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. paleoclaw blocks candidates when:
...
Each plugin must include a `paleoclaw.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `paleoclaw.extensions`:
...
Security guardrail: every `paleoclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `paleoclaw plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `paleoclaw.channel` and
install hints via `paleoclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@paleoclaw/nextcloud-talk",
  "paleoclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@paleoclaw/nextcloud-talk",
...
paleoclaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.paleoclaw/mpm/plugins.json`
- `~/.paleoclaw/mpm/catalog.json`
- `~/.paleoclaw/plugins/catalog.json`

Or point `OPENCLAW_PLUGIN_CATALOG_PATHS` (or `OPENCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "paleoclaw": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, paleoclaw uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `paleoclaw.plugin.json` (`configSchema`).
...
paleoclaw augments `uiHints` at runtime based on discovered plugins:
...
paleoclaw plugins install <path>                 # copy a local file/dir into ~/.paleoclaw/extensions/<id>
paleoclaw plugins install ./extensions/voice-call # relative path ok
paleoclaw plugins install ./plugin.tgz           # install from a local tarball
paleoclaw plugins install ./plugin.zip           # install from a local zip
paleoclaw plugins install -l ./extensions/voice-call # link (no copy) for dev
paleoclaw plugins install @paleoclaw/voice-call # install from npm
paleoclaw plugins install @paleoclaw/voice-call --pin # store exact resolved name@version
...
If stored integrity metadata changes between updates, paleoclaw warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `paleoclaw voicecall`).
...
- Plugin-managed hooks show up in `paleoclaw hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `paleoclaw hooks`; enable/disable the plugin instead.
...
- When disabled, paleoclaw blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside paleoclaw (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `paleoclaw models auth login --provider <id> [--method <id>]`
...
- `config`: The current paleoclaw config
...
- Main package: `paleoclaw` (this repo)
- Plugins: separate npm packages under `@paleoclaw/*` (example: `@paleoclaw/voice-call`)

Publishing contract:

- Plugin `package.json` must include `paleoclaw.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `paleoclaw plugins install <npm-spec>` uses `npm pack`, extracts into `~/.paleoclaw/extensions/<id>/`, and enables it in config.
...
- CLI: `paleoclaw voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `paleoclaw.extensions` points at the built entrypoint (`dist/index.js`).
