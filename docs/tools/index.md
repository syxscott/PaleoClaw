...
read_when:
  ...
  - Retiring or changing `paleoclaw-*` skills
title: "Tools"
---

# Tools (paleoclaw)

paleoclaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `paleoclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `paleoclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, paleoclaw logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:paleoclaw`: all built-in paleoclaw tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`paleoclaw node run`).
...
paleoclaw tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `paleoclaw configure --section web`).
...
Control the dedicated paleoclaw-managed browser.
...
- Quick smoke: `paleoclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `paleoclaw gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, paleoclaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, paleoclaw clamps `tools.sessions.visibility` to `tree`.
