...
The paleoclaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate paleoclaw-managed Chrome profile.
...
paleoclaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the paleoclaw release (npm package) as static files. There is no separate “build” step.

After upgrading paleoclaw:

- Re-run `paleoclaw browser extension install` to refresh the installed files under your paleoclaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

paleoclaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want paleoclaw to control.
...
- `ON`: attached; paleoclaw can drive that tab.
...
Debugging: `paleoclaw sandbox explain`
...
`paleoclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `paleoclaw browser extension install` first to copy the extension to a stable location under your paleoclaw state directory.
...
- **This is not isolated** like the dedicated paleoclaw-managed profile.
