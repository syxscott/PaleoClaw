...
Lobster is a workflow shell that lets paleoclaw run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: paleoclaw runs one Lobster tool call and gets a structured result.
...
paleoclaw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | paleoclaw.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
paleoclaw.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In paleoclaw tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the paleoclaw Gateway (see the [Lobster repo](https://github.com/paleoclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), paleoclaw keeps core tools enabled. To restrict core
...
→ paleoclaw calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ paleoclaw calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls paleoclaw tools that do.
