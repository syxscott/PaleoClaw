...
- **gateway host** → `paleoclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.paleoclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.paleoclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `paleoclaw approvals allowlist ...`).
- `paleoclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `paleoclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.paleoclaw/exec-approvals.json` directly.

CLI: `paleoclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
