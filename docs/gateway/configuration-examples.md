...
```json5
{
  agent: { workspace: "~/.paleoclaw/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.paleoclaw/paleoclaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.paleoclaw/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  cron: {
    enabled: true,
    store: "~/.paleoclaw/cron/cron.json",
    maxConcurrentRuns: 2,
