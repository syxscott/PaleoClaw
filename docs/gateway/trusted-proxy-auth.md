...
read_when:
  ...
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of paleoclaw
...
- You run paleoclaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. paleoclaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. paleoclaw extracts the user identity from the configured header
...
- paleoclaw can stay on loopback HTTP behind the proxy.
...
If paleoclaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://paleoclaw.example.com
    to: http://paleoclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
paleoclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy paleoclaw:18789 {
...
location / {
    ...
    proxy_pass http://paleoclaw:18789;
...
`paleoclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update paleoclaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `paleoclaw security audit` and review findings
