...
paleoclaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
When onboarding runs in interactive mode and you choose SecretRef storage, paleoclaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). paleoclaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/paleoclaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/paleoclaw"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `paleoclaw.json` refs)
...
When reload-time activation fails after a healthy state, paleoclaw enters degraded secrets state.
...
- Strict command paths (for example `paleoclaw memory` remote-memory paths and `paleoclaw qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `paleoclaw status`, `paleoclaw status --all`, `paleoclaw channels status`, `paleoclaw channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `paleoclaw secrets reload`.
...
```bash
paleoclaw secrets audit --check
paleoclaw secrets configure
paleoclaw secrets audit --check
```
...
- plaintext values at rest (`paleoclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `paleoclaw.json` refs)
...
- lets you select supported secret-bearing fields in `paleoclaw.json` plus `auth-profiles.json` for one agent scope
...
- `paleoclaw secrets configure --providers-only`
- `paleoclaw secrets configure --skip-provider-setup`
- `paleoclaw secrets configure --agent <id>`
...
```bash
paleoclaw secrets apply --from /tmp/paleoclaw-secrets-plan.json
paleoclaw secrets apply --from /tmp/paleoclaw-secrets-plan.json --dry-run
```
...
paleoclaw intentionally does not write rollback backups containing historical plaintext secret values.
