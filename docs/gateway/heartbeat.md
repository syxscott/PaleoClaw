...
- During heartbeat runs, paleoclaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `paleoclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, paleoclaw skips the heartbeat run entirely (no model call).
...
```bash
paleoclaw system event --text "Check for urgent follow-ups" --mode now
```
