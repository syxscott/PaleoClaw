...
paleoclaw supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `paleoclaw gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.paleoclaw/.env` so the daemon can read it:

```bash
cat >> ~/.paleoclaw/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `paleoclaw onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.paleoclaw/.env`, systemd/launchd).
...
```bash
paleoclaw models auth paste-token --provider anthropic
paleoclaw models auth paste-token --provider openrouter
```
...
- paleoclaw retries with the next key only for rate-limit errors (for example
...
Run `paleoclaw models status` to confirm which profile is expiring. If the profile
