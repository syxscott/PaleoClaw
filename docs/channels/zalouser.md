...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside paleoclaw.
...
- Install via CLI: `paleoclaw plugins install @paleoclaw/zalouser`
- Or from a source checkout: `paleoclaw plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `paleoclaw channels login --channel zalouser`
...
```bash
paleoclaw directory self --channel zalouser
paleoclaw directory peers list --channel zalouser --query "name"
paleoclaw directory groups list --channel zalouser --query "work"
```
...
- `paleoclaw pairing list zalouser`
- `paleoclaw pairing approve zalouser <code>`
...
- On startup, paleoclaw resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, paleoclaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in paleoclaw state. Example:
...
- paleoclaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, paleoclaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `paleoclaw channels logout --channel zalouser && paleoclaw channels login --channel zalouser`
...
- The channel now runs fully in paleoclaw without external CLI binaries.
