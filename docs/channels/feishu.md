...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects paleoclaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
paleoclaw plugins install @paleoclaw/feishu
```

Local checkout (when running from a git repo):

```bash
paleoclaw plugins install ./extensions/feishu
```
...
If you just installed paleoclaw, run the wizard:
...
2. Configuring app credentials in paleoclaw
...
1. You already ran `paleoclaw channels add` for Feishu
2. The gateway is running (`paleoclaw gateway status`)
...
```bash
paleoclaw pairing approve feishu <CODE>
```
...
2. Run `paleoclaw logs --follow` and look for `chat_id`
...
2. Run `paleoclaw logs --follow` and look for `open_id`
...
| `paleoclaw gateway restart` | Restart gateway service       |
| `paleoclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `paleoclaw logs --follow`
...
5. Ensure the gateway is running: `paleoclaw gateway status`
6. Check logs: `paleoclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.paleoclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.paleoclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
