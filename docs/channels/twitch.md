...
Twitch chat support via IRC connection. paleoclaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
paleoclaw plugins install @paleoclaw/twitch
```

Local checkout (when running from a git repo):

```bash
paleoclaw plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "paleoclaw", // Bot's Twitch account
