...
```bash
paleoclaw plugins install @paleoclaw/msteams
```

Local checkout (when running from a git repo):

```bash
paleoclaw plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
paleoclaw will offer the local install path automatically.
...
3. Configure paleoclaw with those credentials.
...
- Talk to paleoclaw via Teams DMs, group chats, or channels.
...
- On startup, paleoclaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.paleoclaw/paleoclaw.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring paleoclaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `paleoclaw-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `paleoclaw plugins install @paleoclaw/msteams`
   - From a local checkout: `paleoclaw plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "paleoclaw in Teams", "full": "paleoclaw in Teams" },
...
paleoclaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, paleoclaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
paleoclaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `paleoclaw message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.paleoclaw/msteams-polls.json`.
...
```bash
paleoclaw message send --channel msteams \
...
paleoclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
paleoclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
paleoclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
paleoclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
