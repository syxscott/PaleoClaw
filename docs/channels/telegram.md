...
```
    ...
    Telegram does **not** use `paleoclaw channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `paleoclaw doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `paleoclaw doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `paleoclaw logs --follow`
...
- DM messages can carry `message_thread_id`; paleoclaw routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (native drafts + message edits)">
    paleoclaw can stream partial replies in real time:
    ...
    - DM: paleoclaw updates the draft in place (no extra preview message)
    - group/topic: paleoclaw keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), paleoclaw falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, paleoclaw skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, paleoclaw automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, paleoclaw retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - paleoclaw pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.paleoclaw/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, paleoclaw enqueues system events like:
...
```bash
paleoclaw message send --channel telegram --target 123456789 --message "hi"
paleoclaw message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `paleoclaw message poll` and support forum topics:

```bash
paleoclaw message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
paleoclaw message poll --channel telegram --target -1001234567890:topic:42 \
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `paleoclaw channels status` warns when config expects unmentioned group messages.
    - `paleoclaw channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `paleoclaw logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, paleoclaw now retries these as recoverable network errors.
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `paleoclaw doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `paleoclaw doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, paleoclaw falls back to the first normalized account ID and `paleoclaw doctor` warns.
