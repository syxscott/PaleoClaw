...
```bash
paleoclaw plugins install @paleoclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
paleoclaw plugins install ./extensions/mattermost
```
...
4. Configure paleoclaw and start the gateway.
...
Native slash commands are opt-in. When enabled, paleoclaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, paleoclaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as paleoclaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to paleoclaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from paleoclaw, not `404`.
...
- Approve via:
  - `paleoclaw pairing list mattermost`
  - `paleoclaw pairing approve mattermost <CODE>`
...
Use these target formats with `paleoclaw message send` or cron/webhooks:
...
- If `interactions.callbackBaseUrl` is omitted, paleoclaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and paleoclaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="paleoclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"paleoclaw-mattermost-interactions",
...
`paleoclaw message send` and cron/webhook deliveries.
