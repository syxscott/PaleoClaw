...
Matrix is an open, decentralized messaging protocol. paleoclaw connects as a Matrix **user**
...
```bash
paleoclaw plugins install @paleoclaw/matrix
```

Local checkout (when running from a git repo):

```bash
paleoclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `paleoclaw plugins install @paleoclaw/matrix`
   - From a local checkout: `paleoclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: paleoclaw calls the same
     login endpoint, stores the access token in `~/.paleoclaw/credentials/matrix/credentials.json`,
...
- On first connection, paleoclaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  paleoclaw logs a warning.
...
`~/.paleoclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `paleoclaw pairing list matrix`
  - `paleoclaw pairing approve matrix <CODE>`
...
- On startup, paleoclaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
