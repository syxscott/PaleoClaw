...
```bash
paleoclaw plugins install @paleoclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
paleoclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "paleoclaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `paleoclaw pairing list nextcloud-talk`
  - `paleoclaw pairing approve nextcloud-talk <CODE>`
