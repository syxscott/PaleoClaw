...
```bash
paleoclaw plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your paleoclaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in paleoclaw.
...
- Pairing approvals work with:
  - `paleoclaw pairing list synology-chat`
  - `paleoclaw pairing approve synology-chat <CODE>`
...
```bash
paleoclaw message send --channel synology-chat --target 123456 --text "Hello from paleoclaw"
paleoclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
