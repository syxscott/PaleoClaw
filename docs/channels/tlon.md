...
Tlon is a decentralized messenger built on Urbit. paleoclaw connects to your Urbit ship and can
...
```bash
paleoclaw plugins install @paleoclaw/tlon
```

Local checkout (when running from a git repo):

```bash
paleoclaw plugins install ./extensions/tlon
```
...
By default, paleoclaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `paleoclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, paleoclaw replies in-thread.
