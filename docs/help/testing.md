...
paleoclaw has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  - paleoclaw uses Vitest `vmForks` on Node 22/23 for faster unit shards.
  - On Node 24+, paleoclaw automatically falls back to regular `forks` to avoid Node VM linking errors (`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`).
...
### Layer 2: Gateway + dev agent smoke (what “@paleoclaw” actually does)
...
```bash
paleoclaw models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: paleoclaw calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: paleoclaw shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `paleoclaw models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `paleoclaw models list` / model selection.

- Profile store: `~/.paleoclaw/credentials/` (preferred; what “profile keys” means in the tests)
...
- `OPENCLAW_CONFIG_DIR=...` (default: `~/.paleoclaw`) mounted to `/home/node/.paleoclaw`
- `OPENCLAW_WORKSPACE_DIR=...` (default: `~/.paleoclaw/workspace`) mounted to `/home/node/.paleoclaw/workspace`
