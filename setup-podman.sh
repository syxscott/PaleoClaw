#!/usr/bin/env bash
# One-time host setup for rootless paleoclaw in Podman: creates the paleoclaw
...
#   ./scripts/run-paleoclaw-podman.sh launch setup   # onboarding wizard
# Or as the paleoclaw user: sudo -u paleoclaw /home/paleoclaw/run-paleoclaw-podman.sh
# If you used --quadlet, you can also: sudo systemctl --machine paleoclaw@ --user start paleoclaw.service
set -euo pipefail

OPENCLAW_USER="${OPENCLAW_PODMAN_USER:-paleoclaw}"
REPO_PATH="${OPENCLAW_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-paleoclaw-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/paleoclaw.container.in"
...
# Create paleoclaw user (non-login, with home) if missing
...
OPENCLAW_CONFIG="$OPENCLAW_HOME/.paleoclaw"
LAUNCH_SCRIPT_DST="$OPENCLAW_HOME/run-paleoclaw-podman.sh"
...
OPENCLAW_JSON="$OPENCLAW_CONFIG/paleoclaw.json"
...
podman build ${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"} -t paleoclaw:local -f "$REPO_PATH/Dockerfile" "$REPO_PATH"
...
TMP_STAGE_DIR="$(mktemp -d -p "$TMP_IMAGE_DIR" paleoclaw-image.XXXXXX)"
TMP_IMAGE="$TMP_STAGE_DIR/image.tar"
chmod 700 "$TMP_STAGE_DIR"
trap 'rm -rf "$TMP_STAGE_DIR"' EXIT
podman save paleoclaw:local -o "$TMP_IMAGE"
chmod 600 "$TMP_IMAGE"
...
# Optionally install systemd quadlet for paleoclaw user (rootless Podman + systemd)
...
if [[ "$INSTALL_QUADLET" == true && -f "$QUADLET_TEMPLATE" ]]; then
  ...
  sed "s|{{OPENCLAW_HOME}}|$OPENCLAW_HOME_SED|g" "$QUADLET_TEMPLATE" | run_as_openclaw tee "$QUADLET_DIR/paleoclaw.container" >/dev/null
  run_as_openclaw chmod 700 "$OPENCLAW_HOME/.config" "$OPENCLAW_HOME/.config/containers" "$QUADLET_DIR" 2>/dev/null || true
  run_as_openclaw chmod 600 "$QUADLET_DIR/paleoclaw.container" 2>/dev/null || true
  if command -v systemctl &>/dev/null; then
    run_root systemctl --machine "${OPENCLAW_USER}@" --user daemon-reload 2>/dev/null || true
    run_root systemctl --machine "${OPENCLAW_USER}@" --user enable paleoclaw.service 2>/dev/null || true
    run_root systemctl --machine "${OPENCLAW_USER}@" --user start paleoclaw.service 2>/dev/null || true
  fi
fi
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  ...
  echo "  sudo systemctl --machine ${OPENCLAW_USER}@ --user start paleoclaw.service"
  echo "  sudo systemctl --machine ${OPENCLAW_USER}@ --user status paleoclaw.service"
else
