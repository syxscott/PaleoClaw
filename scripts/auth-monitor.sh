...
#
# Suggested cron: */30 * * * * /home/admin/paleoclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send paleoclaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., paleoclaw-alerts)
...
STATE_FILE="$HOME/.paleoclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via paleoclaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use paleoclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via paleoclaw to $NOTIFY_PHONE..."
            paleoclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! paleoclaw is down. Run: ssh l36 '~/paleoclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
