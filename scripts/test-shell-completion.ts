 ...
 * `paleoclaw update`. Use it to verify the completion installation flow
...
\`paleoclaw update\`, \`paleoclaw doctor\`, and \`paleoclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# paleoclaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`paleoclaw completion --install\` later to enable.`));
    return;
  }
