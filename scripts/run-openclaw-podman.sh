...
# Then:
#   ./scripts/run-paleoclaw-podman.sh launch           # Start gateway
#   ./scripts/run-paleoclaw-podman.sh launch setup      # Onboarding wizard
#
# As the paleoclaw user (no repo needed):
...
LAUNCH_SCRIPT="$OPENCLAW_HOME/run-paleoclaw-podman.sh"
...
# --- Step 2: launch (from repo: re-exec as paleoclaw in safe cwd; from paleoclaw home: run container) ---
if [[ "${1:-}" == "launch" ]]; then
  shift
  if [[ -n "${OPENCLAW_UID:-}" && "$(id -u)" -ne "$OPENCLAW_UID" ]]; then
    # Exec as paleoclaw with cwd=/tmp so a nologin user never inherits an invalid cwd.
    exec sudo -u "$OPENCLAW_USER" env HOME="$OPENCLAW_HOME" PATH="$PATH" TERM="${TERM:-}" \
      bash -c 'cd /tmp && exec '"$LAUNCH_SCRIPT"' "$@"' _ "$@"
  fi
  # Already paleoclaw; fall through to container run (with remaining args, e.g. "setup")
fi

# --- Container run (script in paleoclaw home, run as paleoclaw) ---
EFFECTIVE_HOME="${HOME:-}"
...
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.paleoclaw}"
ENV_FILE="${OPENCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${OPENCLAW_PODMAN_CONTAINER:-paleoclaw}"
OPENCLAW_IMAGE="${OPENCLAW_PODMAN_IMAGE:-paleoclaw:local}"
...
# Safe cwd for podman (paleoclaw is nologin; avoid inherited cwd from sudo)
...
CONFIG_JSON="$CONFIG_DIR/paleoclaw.json"
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -v "$CONFIG_DIR:/home/node/.paleoclaw:rw" \
    -v "$WORKSPACE_DIR:/home/node/.paleoclaw/workspace:rw" \
    "${ENV_FILE_ARGS[@]}" \
    "$OPENCLAW_IMAGE" \
