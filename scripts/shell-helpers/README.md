...
Inspired by Simon Willison's [Running paleoclaw in Docker](https://til.simonwillison.net/llms/paleoclaw-docker).
...
On first command, ClawDock auto-detects your paleoclaw directory:

- Checks common paths (`~/paleoclaw`, `~/workspace/paleoclaw`, etc.)
...
```bash
paleoclaw channels login --channel whatsapp --verbose
```
...
2. Configure it in the paleoclaw config
...
- Bash or Zsh shell
- paleoclaw project (from `docker-setup.sh`)
