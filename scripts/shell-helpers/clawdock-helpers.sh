...
# Common paths to check for paleoclaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/paleoclaw"
  "${HOME}/workspace/paleoclaw"
  "${HOME}/projects/paleoclaw"
  "${HOME}/dev/paleoclaw"
  "${HOME}/code/paleoclaw"
  "${HOME}/src/paleoclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found paleoclaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/paleoclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ paleoclaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/paleoclaw/paleoclaw.git ~/paleoclaw"
    echo "  cd ~/paleoclaw && ./docker-setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d paleoclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart paleoclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f paleoclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.paleoclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec paleoclaw-gateway \
    bash -c 'echo "alias paleoclaw=\"./paleoclaw.mjs\"" > /tmp/.bashrc_openclaw && bash --rcfile /tmp/.bashrc_openclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm paleoclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build paleoclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" paleoclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" paleoclaw-gateway \
    bash -c './paleoclaw.mjs config set gateway.remote.token "$TOKEN" && ./paleoclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec paleoclaw-gateway \
    bash -c "./paleoclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart paleoclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm paleoclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec paleoclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'paleoclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for paleoclaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (paleoclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to paleoclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.paleoclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'paleoclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'paleoclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.paleoclaw.ai${_CLR_RESET}"
  echo ""
}
