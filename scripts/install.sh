...
# paleoclaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://paleoclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one paleoclaw."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 paleoclaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://paleoclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.paleoclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_openclaw_paths() {
    ...
    rm -rf "$npm_root"/.paleoclaw-* "$npm_root"/paleoclaw 2>/dev/null || true
}
...
cleanup_openclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/paleoclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/paleoclaw"|"/usr/local/bin/paleoclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/paleoclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale paleoclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing paleoclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing paleoclaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_openclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "paleoclaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*paleoclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            ...
            ui_error "npm failed because an paleoclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
GIT_DIR_DEFAULT="${HOME}/paleoclaw"
...
print_usage() {
    cat <<EOF
paleoclaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://paleoclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/paleoclaw)
...
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://paleoclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://paleoclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://paleoclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected paleoclaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a paleoclaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_openclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"paleoclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://paleoclaw.ai/install.sh | bash"
}
...
ensure_openclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/paleoclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/paleoclaw" ]]; then
        ln -sf "$npm_root/paleoclaw/dist/entry.js" "${npm_bin}/paleoclaw"
        ui_info "Created paleoclaw bin link at ${npm_bin}/paleoclaw"
    fi
    return 0
}

# Check for existing paleoclaw installation
check_existing_openclaw() {
    if [[ -n "$(type -P paleoclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing paleoclaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make paleoclaw show as \"command not found\" in new terminals."
...
warn_openclaw_not_found() {
    ui_warn "Installed, but paleoclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t paleoclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named paleoclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_openclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P paleoclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/paleoclaw" ]]; then
        echo "${npm_bin}/paleoclaw"
        return 0
    fi
...
install_openclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/paleoclaw/paleoclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing paleoclaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing paleoclaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning paleoclaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building paleoclaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/paleoclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/paleoclaw"
    ui_success "paleoclaw wrapper installed to \$HOME/.local/bin/paleoclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view paleoclaw dist-tags.beta 2>/dev/null || true)"
...
install_openclaw() {
    ...
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing paleoclaw v${resolved_version}"
    else
        ui_info "Installing paleoclaw (${OPENCLAW_VERSION})"
    fi
    local install_spec=""
    ...
    if [[ "${OPENCLAW_VERSION}" == "latest" && "${package_name}" == "paleoclaw" ]]; then
        if ! resolve_openclaw_bin &> /dev/null; then
            ui_warn "npm install paleoclaw@latest failed; retrying paleoclaw@next"
            cleanup_npm_openclaw_paths
            install_openclaw_npm "paleoclaw@next"
        fi
    fi

    ensure_openclaw_bin_link || true

    ui_success "paleoclaw installed"
}
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (paleoclaw not on PATH yet)"
        warn_openclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    ...
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.paleoclaw/workspace-${profile}"
    else
        echo "${HOME}/.paleoclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${OPENCLAW_CONFIG_PATH:-$HOME/.paleoclaw/paleoclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run paleoclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but paleoclaw not on PATH; skipping onboarding"
        warn_openclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run paleoclaw onboard to retry"
        return
    }
}
...
resolve_openclaw_version() {
    local version=""
    local claw="${OPENCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v paleoclaw &> /dev/null; then
        claw="$(command -v paleoclaw)"
    fi
    if [[ -n "$claw" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/paleoclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/paleoclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found paleoclaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
    ...
    ui_stage "Installing paleoclaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g paleoclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g paleoclaw 2>/dev/null || true
        ...
        if [[ -x "$HOME/.local/bin/paleoclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/paleoclaw"
    ...
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 paleoclaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 paleoclaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/paleoclaw"
        ui_kv "Update command" "paleoclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://paleoclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running paleoclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
            ...
            ui_info "No TTY; run paleoclaw doctor and paleoclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run paleoclaw onboard later"
        else
            local config_path="${OPENCLAW_CONFIG_PATH:-$HOME/.paleoclaw/paleoclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (paleoclaw not on PATH yet)"
                    warn_openclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run paleoclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v paleoclaw &> /dev/null; then
        local claw="${OPENCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_openclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (paleoclaw daemon restart)"
            else
                ...
                if OPENCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ...
                    ui_warn "Gateway restart failed; try: paleoclaw daemon restart"
                fi
            fi
        fi
    fi
