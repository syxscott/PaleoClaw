...
IMAGE_NAME="paleoclaw-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/paleoclaw-bin:$PATH"
  mkdir -p /tmp/paleoclaw-bin

  cat > /tmp/paleoclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/paleoclaw-bin/systemctl

  cat > /tmp/paleoclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/paleoclaw-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/paleoclaw"
	  npm_root="/tmp/npm-prefix/lib/node_modules/paleoclaw"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/paleoclaw-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/paleoclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
