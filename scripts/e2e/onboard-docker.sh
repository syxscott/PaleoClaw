...
IMAGE_NAME="paleoclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/paleoclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/paleoclaw-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/paleoclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/paleoclaw-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/paleoclaw-e2e-$1.XXXXXX"
  }
  ...
  set_isolated_openclaw_env() {
    ...
    export OPENCLAW_STATE_DIR="$home_dir/.paleoclaw"
    export OPENCLAW_CONFIG_PATH="$OPENCLAW_STATE_DIR/paleoclaw.json"
