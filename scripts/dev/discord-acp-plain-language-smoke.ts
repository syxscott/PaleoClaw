...
type DriverMode = "token" | "webhook" | "paleoclaw";
...
function resolveStateDir(): string {
  ...
  return path.join(home, ".paleoclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver paleoclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) paleoclaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|paleoclaw> Driver transport mode (default: token)\n" +
    ...
    "  --paleoclaw-bin <path>        paleoclaw CLI binary for driver=paleoclaw (default: paleoclaw)\n" +
...
function parseArgs(): Args {
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "paleoclaw"
        ? "paleoclaw"
  ...
  const threadBindingsPath =
    ...
    defaultBindingsPath;
  const openclawBin =
    resolveArg("--paleoclaw-bin") || process.env.OPENCLAW_DISCORD_SMOKE_OPENCLAW_BIN || "paleoclaw";
...
async function openclawCliJson<T>(params: { openclawBin: string; args: string[] }): Promise<T> {
  ...
  if (!stdout) {
    throw new Error(`paleoclaw ${params.args.join(" ")} returned empty stdout`);
  }
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "paleoclaw") {
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `paleoclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("paleoclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from paleoclaw.`,
        diagnostics: {
          bindingCandidates: [
            {
