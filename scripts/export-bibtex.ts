...
function paperToBibtex(paper: Paper, key?: string): string {
  ...
  if (paper.title) {
    entries.push(`  title = {${paper.title}},`);
  }
  if (paper.authors && paper.authors.length > 0) {
    entries.push(`  author = {${formatAuthors(paper.authors)}},`);
  }
  if (paper.year) {
    entries.push(`  year = {${paper.year}},`);
  }
  if (paper.journal) {
    entries.push(`  journal = {${paper.journal}},`);
  }
  if (paper.volume) {
    entries.push(`  volume = {${paper.volume}},`);
  }
  if (paper.issue) {
    entries.push(`  number = {${paper.issue}},`);
  }
  if (paper.pages) {
    entries.push(`  pages = {${paper.pages}},`);
  }
  if (paper.doi) {
    entries.push(`  doi = {${paper.doi}},`);
  }
  if (paper.url) {
    entries.push(`  url = {${paper.url}},`);
  }
  ...
  try {
    ...
    if (args[0] === "--doi") {
      ...
      output =
        ...
        `${sanitizeBibtex(papers[0].title || "ref", papers[0].year)}.bib`;
    } else {
