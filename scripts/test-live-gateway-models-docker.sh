...
IMAGE_NAME="${OPENCLAW_IMAGE:-${CLAWDBOT_IMAGE:-paleoclaw:local}}"
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${CLAWDBOT_CONFIG_DIR:-$HOME/.paleoclaw}}"
WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-${CLAWDBOT_WORKSPACE_DIR:-$HOME/.paleoclaw/workspace}}"
...
docker run --rm -t \
  ...
  -v "$ROOT_DIR":/src:ro \
  -v "$CONFIG_DIR":/home/node/.paleoclaw \
  -v "$WORKSPACE_DIR":/home/node/.paleoclaw/workspace \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  -lc "$LIVE_TEST_CMD"
