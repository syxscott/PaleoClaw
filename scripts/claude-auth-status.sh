...
OPENCLAW_AUTH="$HOME/.paleoclaw/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    paleoclaw models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    jq -n \
        ...
        '{
            ...
            paleoclaw: {status: $bs, expires_at_ms: ($be | tonumber)},
...
echo ""
echo "paleoclaw Auth (~/.paleoclaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'paleoclaw doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active paleoclaw >/dev/null 2>&1; then
    echo -e "paleoclaw service: ${GREEN}running${NC}"
else
    echo -e "paleoclaw service: ${RED}NOT running${NC}"
fi
