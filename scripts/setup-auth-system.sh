...
echo "=== paleoclaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'paleoclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  paleoclaw message: Send warning via paleoclaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/paleoclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/paleoclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/paleoclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now paleoclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status paleoclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
