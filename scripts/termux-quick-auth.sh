...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/paleoclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "paleoclaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/paleoclaw/scripts/mobile-reauth.sh'" --id paleoclaw-auth
        ;;
    *)
