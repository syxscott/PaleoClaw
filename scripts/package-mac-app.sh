...
# Build and bundle paleoclaw into a minimal .app we can open.
# Outputs to dist/paleoclaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/paleoclaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="paleoclaw"
BUNDLE_ID="${BUNDLE_ID:-ai.paleoclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/paleoclaw/paleoclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/paleoclaw/Resources/Info.plist"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/paleoclaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/paleoclaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/paleoclaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/paleoclaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/paleoclaw/Resources/paleoclaw.icns" "$APP_ROOT/Contents/Resources/paleoclaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/paleoclaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "⏹  Stopping any running paleoclaw"
killall -q paleoclaw 2>/dev/null || true
