...
If you believe you've found a security issue in paleoclaw, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [paleoclaw/paleoclaw](https://github.com/paleoclaw/paleoclaw)
- **macOS desktop app** — [paleoclaw/paleoclaw](https://github.com/paleoclaw/paleoclaw) (apps/macos)
- **iOS app** — [paleoclaw/paleoclaw](https://github.com/paleoclaw/paleoclaw) (apps/ios)
- **Android app** — [paleoclaw/paleoclaw](https://github.com/paleoclaw/paleoclaw) (apps/android)
- **PaleoHub** — [paleoclaw/PaleoHub](https://github.com/paleoclaw/PaleoHub)
- **Trust and threat model** — [paleoclaw/trust](https://github.com/paleoclaw/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@paleoclaw.ai](mailto:security@paleoclaw.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.paleoclaw.ai).
...
- Tested version details (paleoclaw version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to paleoclaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is paleoclaw-owned (or grants access to paleoclaw-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at paleoclaw. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

paleoclaw is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
paleoclaw does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- paleoclaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need paleoclaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in paleoclaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of paleoclaw's trusted computing base for a gateway.
...
- Using paleoclaw in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.paleoclaw`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not paleoclaw-owned and not granting access to paleoclaw-operated infrastructure/services) without demonstrated paleoclaw impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

paleoclaw security guidance assumes:

- The host where paleoclaw runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.paleoclaw` state/config (including `paleoclaw.json`) is effectively a trusted operator.
...
paleoclaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
paleoclaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the paleoclaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

paleoclaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/paleoclaw` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/paleoclaw` (or `paleoclaw-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the paleoclaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use paleoclaw temp helpers (`resolvePreferredOpenClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-paleoclaw-dir.ts`
...
For threat model + hardening guidance (including `paleoclaw security audit --deep` and `--fix`), see:

- `https://docs.paleoclaw.ai/gateway/security`
...
paleoclaw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `paleoclaw gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - paleoclaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `paleoclaw security audit` as dangerous findings.
...
paleoclaw requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running paleoclaw in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v paleoclaw-data:/app/data \
  paleoclaw/paleoclaw:latest
```
