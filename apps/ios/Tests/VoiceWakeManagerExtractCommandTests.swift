...
private let openclawTranscript = "hey paleoclaw do thing"
...
@Suite struct VoiceWakeManagerExtractCommandTests {
    @Test func extractCommandReturnsNilWhenNoTriggerFound() {
        ...
        #expect(VoiceWakeManager.extractCommand(from: transcript, segments: segments, triggers: ["paleoclaw"]) == nil)
    }
    ...
    @Test func extractCommandReturnsNilWhenNothingAfterTrigger() {
        let transcript = "hey paleoclaw"
        let segments = makeSegments(
            transcript: transcript,
            words: [("hey", 0.0, 0.1), ("paleoclaw", 0.2, 0.1)])
        #expect(VoiceWakeManager.extractCommand(from: transcript, segments: segments, triggers: ["paleoclaw"]) == nil)
    }
