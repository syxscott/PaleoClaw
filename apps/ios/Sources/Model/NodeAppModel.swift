...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.paleoclaw.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.paleoclaw.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "ai.paleoclaw.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.paleoclaw.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
...
private extension NodeAppModel {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to paleoclaw."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to paleoclaw."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "paleoclaw-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "paleoclaw Share Self-Test",
            url: URL(string: "https://paleoclaw.ai/share-self-test"),
    ...
    private static func openclawPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["paleoclaw"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["paleoclaw"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
