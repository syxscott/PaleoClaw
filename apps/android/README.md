...
```bash
pnpm paleoclaw gateway --port 18789 --verbose
```
...
2) Android app is connected to that gateway and `paleoclaw nodes status` shows it as paired + connected.
...
- `OPENCLAW_ANDROID_GATEWAY_URL=ws://...` (default: from your local paleoclaw config)
...
- `pairing required` before tests start:
  - approve pending device pairing (`paleoclaw devices approve --latest`) and rerun.
