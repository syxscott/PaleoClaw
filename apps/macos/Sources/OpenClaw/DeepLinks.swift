...
private let deepLinkLogger = Logger(subsystem: "ai.paleoclaw", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "paleoclaw is paused", message: "Unpause paleoclaw to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run paleoclaw agent?", message: body) else { return }
        }
