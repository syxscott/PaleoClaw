...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.paleoclaw", category: "gateway.env")
        ...
        case let .success(runtime):
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "paleoclaw CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "paleoclaw@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "paleoclaw@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "paleoclaw@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "paleoclaw@\(target)"])
            }

        statusHandler("Installing paleoclaw@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed paleoclaw@\(target)")
        } else {
