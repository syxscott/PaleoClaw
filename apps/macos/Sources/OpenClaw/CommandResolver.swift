...
enum CommandResolver {
    private static let projectRootDefaultsKey = "paleoclaw.gatewayProjectRootPath"
    private static let helperName = "paleoclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openclawEntry = root.appendingPathComponent("paleoclaw.mjs").path
        if FileManager().isReadableFile(atPath: openclawEntry) { return openclawEntry }
        let binEntry = root.appendingPathComponent("bin/paleoclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/paleoclaw")
    ...
    private static func openclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".paleoclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("paleoclaw.mjs").path,
            root.appendingPathComponent("bin/paleoclaw.js").path,
        ]
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "paleoclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            paleoclaw entrypoint missing (looked for dist/index.js or paleoclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real paleoclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/paleoclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v paleoclaw >/dev/null 2>&1; then
          CLI="$(command -v paleoclaw)"
          paleoclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/paleoclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/paleoclaw.mjs"
            node "$PRJ/paleoclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/paleoclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/paleoclaw.js"
            node "$PRJ/bin/paleoclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent paleoclaw"
          pnpm --silent paleoclaw \(quotedArgs);
        else
          echo "paleoclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
