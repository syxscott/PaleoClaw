...
struct OpenClawMacCLI {
    static func main() async {
        ...
        default:
            fputs("paleoclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      paleoclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      paleoclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      paleoclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      paleoclaw-mac connect
      paleoclaw-mac connect --url ws://127.0.0.1:18789 --json
      paleoclaw-mac discover --timeout 3000 --json
      paleoclaw-mac wizard --mode local
    """)
}
