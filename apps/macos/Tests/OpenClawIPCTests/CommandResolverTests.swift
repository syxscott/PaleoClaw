...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersOpenClawBinary() throws {
        ...
        let openclawPath = tmp.appendingPathComponent("node_modules/.bin/paleoclaw")
    ...
    @Test func fallsBackToNodeAndScript() throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/paleoclaw.js")
    ...
    @Test func prefersOpenClawBinaryOverPnpm() throws {
        ...
        let openclawPath = binDir.appendingPathComponent("paleoclaw")
    ...
    @Test func fallsBackToPnpm() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "paleoclaw", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "paleoclaw", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() {
        ...
        defaults.set("paleoclaw@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/paleoclaw", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "paleoclaw@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/paleoclaw'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("paleoclaw"))
