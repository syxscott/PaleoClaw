...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["paleoclaw"]
        let text = "hey paleoclaw"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey paleoclaw do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["paleoclaw"], minPostTriggerGap: 0.3)
