...
IMAGE_NAME="${OPENCLAW_IMAGE:-paleoclaw:local}"
...
read_config_gateway_token() {
  local config_path="$OPENCLAW_CONFIG_DIR/paleoclaw.json"
...
ensure_control_ui_allowed_origins() {
  ...
  current_allowed_origins="$(
    docker compose "${COMPOSE_ARGS[@]}" run --rm paleoclaw-cli \
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$HOME/.paleoclaw}"
OPENCLAW_WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-$HOME/.paleoclaw/workspace}"
...
if [[ -z "${OPENCLAW_GATEWAY_TOKEN:-}" ]]; then
  ...
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ...
    echo "Reusing gateway token from $OPENCLAW_CONFIG_DIR/paleoclaw.json"
  else
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${OPENCLAW_CONFIG_DIR}:/home/node/.paleoclaw"
    gateway_workspace_mount="${OPENCLAW_WORKSPACE_DIR}:/home/node/.paleoclaw/workspace"
...
if [[ "$IMAGE_NAME" == "paleoclaw:local" ]]; then
...
# After fixing the config dir, only the paleoclaw metadata subdirectory
# (.paleoclaw/) inside the workspace gets chowned, not the user's project files.
docker compose "${COMPOSE_ARGS[@]}" run --rm --user root --entrypoint sh paleoclaw-cli -c \
  'find /home/node/.paleoclaw -xdev -exec chown node:node {} +; \
   [ -d /home/node/.paleoclaw/workspace/.paleoclaw ] && chown -R node:node /home/node/.paleoclaw/workspace/.paleoclaw || true'
...
echo ""
docker compose "${COMPOSE_ARGS[@]}" run --rm paleoclaw-cli onboard --mode local --no-install-daemon
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm paleoclaw-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm paleoclaw-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm paleoclaw-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.paleoclaw.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d paleoclaw-gateway
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: paleoclaw-sandbox:bookworm-slim"
    docker build \
      -t "paleoclaw-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker paleoclaw-gateway --version >/dev/null 2>&1; then
    ...
    echo "  or use a local build (OPENCLAW_IMAGE=paleoclaw:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in paleoclaw config.
  sandbox_config_ok=true
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --no-deps paleoclaw-cli \
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.paleoclaw.ai/gateway/sandboxing"
    ...
    if ! docker compose "${BASE_COMPOSE_ARGS[@]}" run --rm --no-deps paleoclaw-cli \
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate paleoclaw-gateway
  fi
else
  ...
  # config alone.
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm paleoclaw-cli \
    config set agents.defaults.sandbox.mode "off" >/dev/null; then
...
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f paleoclaw-gateway"
echo "  ${COMPOSE_HINT} exec paleoclaw-gateway node dist/index.js health --token \"$OPENCLAW_GATEWAY_TOKEN\""
