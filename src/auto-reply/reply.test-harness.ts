...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".paleoclaw", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPENCLAW_STATE_DIR = path.join(home, ".paleoclaw");
    process.env.OPENCLAW_AGENT_DIR = path.join(home, ".paleoclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".paleoclaw", "agent");
  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "paleoclaw"),
      },
    },
    channels: {
      whatsapp: {
