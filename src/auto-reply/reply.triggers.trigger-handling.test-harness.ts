...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPENCLAW_STATE_DIR = join(home, ".paleoclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "paleoclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".paleoclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
