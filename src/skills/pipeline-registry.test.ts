...
describe("runPipeline", () => {
  it("runs pre-skills (memoized) then steps in order", async () => {
    const cEntry: PipelineEntry = {
      id: "c",
      type: "pipeline",
      steps: [{ tool: "geo_variables", params: {} }],
    };
    const bEntry: PipelineEntry = {
      id: "b",
      type: "pipeline",
      pre: ["c"],
      steps: [{ tool: "pbdb_query", params: {} }],
    };
    const aEntry: PipelineEntry = {
      id: "a",
      type: "pipeline",
      pre: ["b", "c"],
      steps: [{ tool: "time_rules", params: {} }],
    };
    ...
    expect(report.steps).toEqual([
      { tool: "time_rules", status: "ok", result: "time_rules-result" },
    ]);
  });
