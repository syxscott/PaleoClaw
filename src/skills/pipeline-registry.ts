...
function extractEntryList(parsed: unknown): unknown[] | null {
  ...
  if (parsed && typeof parsed === 'object' && Array.isArray((parsed as Record<string, unknown>).skills)) {
...
function validateEntry(item: unknown): PipelineEntry | string {
  ...
  if (preRaw !== undefined) {
    if (!Array.isArray(preRaw) || preRaw.some((preId) => typeof preId !== 'string' || !preId.trim())) {
...
export function loadPipelineRegistry(registryPath: string): PipelineRegistry {
  ...
  if (!list) {
    result.errors.push({ reason: 'registry must be an array of entries or an object with a "skills" array' });
    return result;
  }
  ...
  list.forEach((item, index) => {
    ...
    if (seenIds.has(validated.id)) {
      result.errors.push({ id: validated.id, reason: `entry ${index}: duplicate skill id '${validated.id}'` });
      return;
    }
