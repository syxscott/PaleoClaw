...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update paleoclaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["paleoclaw update", "Update a source checkout (git)"],
        ["paleoclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["paleoclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["paleoclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["paleoclaw update --dry-run", "Preview actions without changing anything"],
        ["paleoclaw update --no-restart", "Update without restarting the service"],
        ["paleoclaw update --json", "Output result as JSON"],
        ["paleoclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["paleoclaw update wizard", "Interactive update wizard"],
        ["paleoclaw --update", "Shorthand for paleoclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run paleoclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.paleoclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["paleoclaw update status", "Show channel + version status."],
          ["paleoclaw update status --json", "JSON output."],
          ["paleoclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.paleoclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
