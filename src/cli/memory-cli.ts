    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["paleoclaw memory status", "Show index and provider status."],
          ["paleoclaw memory status --deep", "Probe embedding provider readiness."],
          ["paleoclaw memory index --force", "Force a full reindex."],
          ['paleoclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'paleoclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["paleoclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.paleoclaw.ai/cli/memory")}\n`,
    );
