    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["paleoclaw nodes status", "List known nodes with live status."],
          ["paleoclaw nodes pairing pending", "Show pending node pairing requests."],
          ['paleoclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["paleoclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.paleoclaw.ai/cli/nodes")}\n`,
    );
