...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPENCLAW_STATE_DIR: "/tmp/paleoclaw-daemon",
    OPENCLAW_CONFIG_PATH: "/tmp/paleoclaw-daemon/paleoclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPENCLAW_STATE_DIR ?? "/tmp/paleoclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPENCLAW_CONFIG_PATH ?? `${stateDir}/paleoclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/paleoclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENCLAW_STATE_DIR = "/tmp/paleoclaw-cli";
    process.env.OPENCLAW_CONFIG_PATH = "/tmp/paleoclaw-cli/paleoclaw.json";
