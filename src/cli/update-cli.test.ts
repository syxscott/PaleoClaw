...
vi.mock("../infra/paleoclaw-root.js", () => ({
...
const { resolveOpenClawPackageRoot } = await import("../infra/paleoclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/paleoclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/paleoclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("paleoclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    readPackageName.mockResolvedValue("paleoclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/paleoclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "paleoclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("paleoclaw update status");
  });
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("paleoclaw-updated-root");
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("paleoclaw-update-wizard");
