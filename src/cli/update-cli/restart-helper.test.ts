...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'paleoclaw-gateway.service'");
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.paleoclaw.gateway'");
    ...
    it("uses OPENCLAW_LAUNCHD_LABEL override on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        OPENCLAW_PROFILE: "default",
        OPENCLAW_LAUNCHD_LABEL: "com.custom.paleoclaw",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.paleoclaw'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "paleoclaw Gateway"');
      expect(content).toContain('schtasks /Run /TN "paleoclaw Gateway"');
    ...
    it("uses OPENCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        OPENCLAW_PROFILE: "default",
        OPENCLAW_WINDOWS_TASK_NAME: "paleoclaw Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "paleoclaw Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "paleoclaw Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      ...
      expect(content).toContain("paleoclaw-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      expect(content).toContain("gui/502/ai.paleoclaw.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "paleoclaw Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        OPENCLAW_LAUNCHD_LABEL: "ai.paleoclaw.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.paleoclaw.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
