...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.paleoclaw-live-tool-probe.${nonceA}.txt`);
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-live-"));
  const tempConfigPath = path.join(tempDir, "paleoclaw.json");
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "paleoclaw live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.paleoclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
