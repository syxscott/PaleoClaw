...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-paleoclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-paleoclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-paleoclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-paleoclaw-thread-id")?.trim() || undefined;
