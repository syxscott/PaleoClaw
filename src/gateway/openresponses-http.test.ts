  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "paleoclaw", input: "hi" }),
      });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "paleoclaw:beta", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "paleoclaw", input: "hi" },
        { "x-paleoclaw-message-channel": "custom-client-channel" },
      );
