...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-secrets-audit-"));
  const stateDir = path.join(rootDir, ".paleoclaw");
...
describe("secrets audit", () => {
  ...
  it("does not flag non-sensitive routing headers in paleoclaw config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
