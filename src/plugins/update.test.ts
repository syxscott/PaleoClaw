...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-paleoclaw",
      targetDir: "/tmp/opik-paleoclaw",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-paleoclaw": {
              source: "npm",
              spec: "@opik/opik-paleoclaw",
              integrity: "sha512-old",
              installPath: "/tmp/opik-paleoclaw",
            },
          },
        },
      },
      pluginIds: ["opik-paleoclaw"],
      dryRun: true,
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-paleoclaw": {
              source: "npm",
              spec: "@opik/opik-paleoclaw@0.2.5",
              integrity: "sha512-old",
              installPath: "/tmp/opik-paleoclaw",
            },
          },
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @paleoclaw/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @paleoclaw/missing.",
      },
    ]);
  });
