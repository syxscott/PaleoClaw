...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `paleoclaw-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "paleoclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "paleoclaw.plugin.json");
  const linkedManifest = path.join(rootDir, "paleoclaw.plugin.json");
