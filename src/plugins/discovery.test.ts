...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `paleoclaw-plugins-${randomUUID()}`);
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      paleoclaw: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverOpenClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".paleoclaw", "extensions");
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@paleoclaw/voice-call",
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@paleoclaw/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@paleoclaw/escape-pack",
