...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing paleoclaw.extensions; update the plugin package to include paleoclaw.extensions (for example ["./dist/index.js"]). See https://docs.paleoclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-paleoclaw-extensions';
...
function ensureOpenClawExtensions(params: { manifest: PackageManifest }):
  ...
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json paleoclaw.extensions is empty",
...
async function installPluginFromPackageDir(
  ...
  // Prefer the canonical `id` from paleoclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-paleoclaw"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "paleoclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "paleoclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "paleoclaw-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "paleoclaw-npm-pack-",
    spec,
    timeoutMs,
