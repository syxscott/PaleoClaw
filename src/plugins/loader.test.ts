...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "paleoclaw-plugin-"));
let tempDirIndex = 0;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "paleoclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
      ...
      JSON.stringify(
        {
          ...
          paleoclaw: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
describe("loadOpenClawPlugins", () => {
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "paleoclaw.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "paleoclaw.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("paleoclaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
