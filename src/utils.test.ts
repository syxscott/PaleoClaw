...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("paleoclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("paleoclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("paleoclaw-lid-a-", (first) => {
      withTempDirSync("paleoclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.paleoclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "paleoclaw-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENCLAW_HOME over HOME", () => {
    vi.stubEnv("OPENCLAW_HOME", "/srv/paleoclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/paleoclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENCLAW_HOME prefix when OPENCLAW_HOME is set", () => {
    ...
    expect(shortenHomePath(`${path.resolve("/srv/paleoclaw-home")}/.paleoclaw/paleoclaw.json`)).toBe(
      "$OPENCLAW_HOME/.paleoclaw/paleoclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENCLAW_HOME replacement when OPENCLAW_HOME is set", () => {
    ...
    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/paleoclaw-home")}/.paleoclaw/paleoclaw.json`),
    ).toBe("config: $OPENCLAW_HOME/.paleoclaw/paleoclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/paleoclaw")).toBe(path.resolve(os.homedir(), "paleoclaw"));
  });
  ...
  it("prefers OPENCLAW_HOME for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/paleoclaw")).toBe(path.resolve("/srv/paleoclaw-home", "paleoclaw"));
