...
export class SqliteSessionStore implements SessionStoreLike {
  ...
  private repairFtsIfDesynced(): void {
    try {
      const msgRow = asRow(this.db.prepare("SELECT COUNT(*) AS count FROM messages").get()) as
        | { count: number }
        | undefined;
      const ftsRow = asRow(this.db.prepare("SELECT COUNT(*) AS count FROM messages_fts").get()) as
        | { count: number }
        | undefined;
  ...
  private readMessages(sessionId: string): SessionMessage[] {
    const rows = asRows<MessageRow>(
      this.db
        .prepare(
          "SELECT role, content, ts, seq, metadata FROM messages WHERE session_id = ? ORDER BY seq ASC",
        )
        .all(sessionId),
    );
    return rows.map((row) => {
  ...
  private readRecord(sessionId: string): SessionRecord {
    const row = asRow(
      this.db
        .prepare(
          "SELECT id, title, title_source, created_at, updated_at, meta FROM sessions WHERE id = ?",
        )
        .get(sessionId),
  ...
  upsertSession(sessionId: string, title?: string, tags: string[] = []): SessionRecord {
    ...
    const exists = asRow(this.db.prepare("SELECT id FROM sessions WHERE id = ?").get(id)) as
      | SessionRow
      | undefined;
    if (exists) {
      ...
      this.db
        .prepare(
          "UPDATE sessions SET title = ?, title_source = ?, updated_at = ?, meta = ? WHERE id = ?",
        )
    ...
    this.db
      .prepare(
        "INSERT INTO sessions (id, title, title_source, created_at, updated_at, meta) VALUES (?, ?, ?, ?, ?, ?)",
      )
      .run(
        id,
  ...
  appendMessage(
    ...
    this.withTransaction(() => {
      ...
      this.db
        .prepare(
          "INSERT INTO messages (session_id, role, content, ts, seq, metadata) VALUES (?, ?, ?, ?, ?, ?)",
        )
  ...
  listSessions(limit = 20): SessionRecord[] {
    const rows = asRows<SessionRow>(
      this.db
        .prepare(
          "SELECT id, title, title_source, created_at, updated_at, meta FROM sessions ORDER BY updated_at DESC LIMIT ?",
        )
  ...
  private searchWithFts(tokens: string[], limit: number): SessionSearchHit[] {
    ...
    const ftsStmt = this.db.prepare(
      "SELECT session_id, seq FROM messages_fts WHERE messages_fts MATCH ?",
    );
  ...
  private buildHits(tokens: string[], matches: MatchCounts, limit: number): SessionSearchHit[] {
    ...
    for (const [sessionId, perSession] of matches) {
      for (const [seq, count] of perSession) {
        const row = asRow(rowStmt.get(sessionId, seq)) as
          | { content: string; ts: string; title: string }
          | undefined;
        if (!row) {
          continue;
        }
  ...
  getStatus(): { root: string; count: number } {
    const row = asRow(this.db.prepare("SELECT COUNT(*) AS count FROM sessions").get()) as
      | { count: number }
      | undefined;
    return {
      root: this.dbPath,
