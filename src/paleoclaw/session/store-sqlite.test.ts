...
function createSqliteStore(options: ConstructorParameters<typeof SqliteSessionStore>[1] = {}): SqliteSessionStore {
...
describe('SqliteSessionStore', () => {
  ...
  it('runs WAL pragmas by default and supports disabling WAL', () => {
    ...
    const walMode = rawDb(walStore).prepare('PRAGMA journal_mode').get() as { journal_mode: string };
    expect(String(walMode.journal_mode).toLowerCase()).toBe('wal');

    const deleteStore = createSqliteStore({ wal: false });
    const deleteMode = rawDb(deleteStore).prepare('PRAGMA journal_mode').get() as { journal_mode: string };
