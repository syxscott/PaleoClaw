...
const TAXON_SUFFIX_RE =
  /[a-z]{3,}(idae|inae|aceae|opsida|phyta|mycota|formes|oidea|aceae|omorpha)$/i;

function isLikelyTaxon(word: string): boolean {
  if (!word) {
    return false;
  }
  if (TAXON_STOPWORDS.has(word)) {
    return false;
  }
  if (GEOLOGICAL_PERIODS.has(word)) {
    return false;
  }
  if (word.length < 6 && !TAXON_SUFFIX_RE.test(word)) {
    return false;
  }
  return true;
}
