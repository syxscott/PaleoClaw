...
describe("resolveAutoToolPlan - taxon extraction", () => {
  ...
  it('skips the leading verb "Use" and extracts the actual taxon', () => {
    const plans = resolveAutoToolPlan(
      "Use PBDB to find Allosaurus occurrences in Morrison Formation",
    );
  ...
  it("returns CrossRef plan for literature prompts", () => {
    const plans = resolveAutoToolPlan(
      "Find DOI and citation metadata for Jurassic theropod papers via CrossRef",
    );
