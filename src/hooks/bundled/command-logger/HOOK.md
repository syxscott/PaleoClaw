...
2. **Appends to log file** - Writes a JSON line to `~/.paleoclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.paleoclaw/logs/commands.log ~/.paleoclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/paleoclaw`:
   ```
   /home/username/.paleoclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
