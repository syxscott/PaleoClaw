...
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "paleoclaw-root");
...
describe("resolveOpenClawPackageRoot", () => {
  let resolveOpenClawPackageRoot: typeof import("./paleoclaw-root.js").resolveOpenClawPackageRoot;
  let resolveOpenClawPackageRootSync: typeof import("./paleoclaw-root.js").resolveOpenClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveOpenClawPackageRoot, resolveOpenClawPackageRootSync } =
      await import("./paleoclaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "paleoclaw");
    const pkgRoot = path.join(project, "node_modules", "paleoclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "paleoclaw" }));
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "paleoclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "paleoclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "paleoclaw" }));
  ...
  it("returns null for non-paleoclaw package roots", async () => {
    const pkgRoot = fx("not-paleoclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-paleoclaw" }));
