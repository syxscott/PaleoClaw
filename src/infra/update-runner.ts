...
const DEFAULT_PACKAGE_NAME = "paleoclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["paleoclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "paleoclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("paleoclaw doctor", doctorArgv, gitRoot, { OPENCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
