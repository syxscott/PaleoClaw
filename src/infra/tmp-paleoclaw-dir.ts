// Temp-dir resolution for PaleoClaw shared sandbox/browser state (POSIX-only preferred path).
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

export const POSIX_PALEOCLAW_TMP_DIR = "/tmp/paleoclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredPaleoClawTmpDirOptions = {
...
/**
 * PaleoClaw port of upstream tmp-openclaw-dir.ts. Exported under the historical
 * OpenClaw symbol name because existing call sites (sandbox paths, browser paths)
 * still import it by that name.
 */
export function resolvePreferredOpenClawTmpDir(
  options: ResolvePreferredPaleoClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "paleoclaw" : `paleoclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[paleoclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const existingPreferredState = resolveDirState(POSIX_PALEOCLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_PALEOCLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_PALEOCLAW_TMP_DIR)) {
      return POSIX_PALEOCLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_PALEOCLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_PALEOCLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_PALEOCLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_PALEOCLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_PALEOCLAW_TMP_DIR;
