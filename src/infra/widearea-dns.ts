...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*paleoclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "paleoclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "paleoclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "paleoclaw-gw");
  ...
  return `; paleoclaw-content-hash: ${contentHash}\n${contentBody}`;
}
