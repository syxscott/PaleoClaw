...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      domain: "paleoclaw.internal.",
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (paleoclaw)",
    ...
    expect(txt).toContain(`$ORIGIN paleoclaw.internal.`);
    ...
    expect(txt).toContain(`displayName=Mac Studio (paleoclaw)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/paleoclaw`);
  });
