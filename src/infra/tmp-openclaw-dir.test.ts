...
function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `paleoclaw-${uid}`);
}
...
describe("resolvePreferredOpenClawTmpDir", () => {
  it("prefers /tmp/paleoclaw when it already exists and is writable", () => {
  ...
  it("prefers /tmp/paleoclaw when it does not exist but /tmp is writable", () => {
  ...
  it("falls back to os.tmpdir()/paleoclaw when /tmp/paleoclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/paleoclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/paleoclaw is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/paleoclaw is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/paleoclaw is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback paleoclaw temp dir/);
  });
