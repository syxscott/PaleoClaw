...
function expectLaunchdKickstartSupervised(params?: { launchJobLabel?: string }) {
  ...
  process.env.OPENCLAW_LAUNCHD_LABEL = "ai.paleoclaw.gateway";
...
describe("restartGatewayProcessWithFreshPid", () => {
  ...
  it("returns supervised when launchd hints are present on macOS", () => {
    ...
    setPlatform("darwin");
    process.env.LAUNCH_JOB_LABEL = "ai.paleoclaw.gateway";
    triggerOpenClawRestartMock.mockReturnValue({ ok: true, method: "launchctl" });
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(triggerOpenClawRestartMock).toHaveBeenCalledOnce();
  ...
  it("runs launchd kickstart helper on macOS when launchd label is set", () => {
    expectLaunchdKickstartSupervised({ launchJobLabel: "ai.paleoclaw.gateway" });
  });
  ...
  it("returns supervised when OPENCLAW_SYSTEMD_UNIT is set", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.env.OPENCLAW_SYSTEMD_UNIT = "paleoclaw-gateway.service";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("returns supervised when paleoclaw gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.OPENCLAW_SERVICE_MARKER = "paleoclaw";
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.OPENCLAW_TASK_SCRIPT = "C:\\paleoclaw\\node.cmd";
