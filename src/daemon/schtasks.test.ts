  ...
  it.each([
    {
      ...
      expected: path.join("C:\\Users\\test", ".paleoclaw", "gateway.cmd"),
    },
    {
      ...
      expected: path.join("C:\\Users\\test", ".paleoclaw-jbphoenix", "gateway.cmd"),
    },
    {
      ...
      env: {
        ...
        OPENCLAW_STATE_DIR: "C:\\State\\paleoclaw",
      },
      expected: path.join("C:\\State\\paleoclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", OPENCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".paleoclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem paleoclaw Gateway",
          "cd /d C:\\Projects\\paleoclaw",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\paleoclaw",
          environment: {
            NODE_ENV: "production",
            OPENCLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\paleoclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\paleoclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\paleoclaw Share\\node.exe" "\\\\fileserver\\paleoclaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\paleoclaw Share\\node.exe",
            "\\\\fileserver\\paleoclaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
