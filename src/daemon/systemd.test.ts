...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false when systemctl is not present", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/paleoclaw-test-home" } });
  ...
  it("calls systemctl is-enabled when systemctl is present", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "paleoclaw-gateway.service"]);
      cb(null, "enabled", "");
    });
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/paleoclaw-test-home" } });
  ...
  it("returns false for the WSL2 Ubuntu 24.04 wrapper-only is-enabled failure", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "paleoclaw-gateway.service"]);
      const err = new Error(
        "Command failed: systemctl --user is-enabled paleoclaw-gateway.service",
    ...
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/paleoclaw-test-home" } }),
    ).rejects.toThrow(
      "systemctl is-enabled unavailable: Command failed: systemctl --user is-enabled paleoclaw-gateway.service",
    );
  });
  ...
  it("returns false when is-enabled cannot connect to the user bus without machine fallback", async () => {
    ...
    await expect(
      isSystemdServiceEnabled({
        env: { HOME: "/tmp/paleoclaw-test-home", USER: "", LOGNAME: "" },
      }),
  ...
  it("returns false when both direct and machine-scope is-enabled checks report bus unavailability", async () => {
    ...
    await expect(
      isSystemdServiceEnabled({
        env: { HOME: "/tmp/paleoclaw-test-home", USER: "debian" },
      }),
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "paleoclaw-gateway.service"]);
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled paleoclaw-gateway.service"),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    {
      ...
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/paleoclaw-gateway.service",
    },
    {
      ...
      expected: "/home/test/.config/systemd/user/paleoclaw-gateway-jbphoenix.service",
    },
    {
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/paleoclaw gateway start --name "My Bot"')).toEqual([
      "/usr/bin/paleoclaw",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('paleoclaw --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["paleoclaw", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('paleoclaw --path "C:\\\\Program Files\\\\paleoclaw"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["paleoclaw", "--path", "C:\\\\Program Files\\\\paleoclaw"]);

    expect(
      splitArgsPreservingQuotes('paleoclaw --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["paleoclaw", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/paleoclaw gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads OPENCLAW_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/paleoclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/paleoclaw gateway run",
          "EnvironmentFile=%h/.paleoclaw/.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.paleoclaw/.env") {
  ...
  it("ignores missing optional EnvironmentFile entries", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/paleoclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/paleoclaw gateway run",
          "EnvironmentFile=-%h/.paleoclaw/missing.env",
        ].join("\n");
      }
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.programArguments).toEqual(["/usr/bin/paleoclaw", "gateway", "run"]);
    ...
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/paleoclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/paleoclaw gateway run",
          'EnvironmentFile=%h/.paleoclaw/first.env "%h/.paleoclaw/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.paleoclaw/first.env") {
        return "OPENCLAW_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.paleoclaw/second env.env") {
    ...
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/paleoclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/paleoclaw gateway run",
          "EnvironmentFile=%h/.paleoclaw/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.paleoclaw/gateway.env") {
        return [
          "# comment",
          "; another comment",
...
describe("systemd service control", () => {
  ...
  it("stops the resolved user unit", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "stop", "paleoclaw-gateway.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "paleoclaw-gateway-work.service"]);
        cb(null, "", "");
      });
  ...
  it("keeps direct --user scope when SUDO_USER is root", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "paleoclaw-gateway.service"]);
        cb(null, "", "");
      });
