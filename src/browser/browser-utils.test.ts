...
describe("cdp.helpers", () => {
  ...
  it("does not add relay header for unknown loopback ports", () => {
    ...
    expect(headers["x-paleoclaw-relay-token"]).toBeUndefined();
  });
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    try {
      ...
      expect(headers["x-paleoclaw-relay-token"]).toBeTruthy();
      expect(headers["x-paleoclaw-relay-token"]).not.toBe("test-gateway-token");
    } finally {
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!paleoclaw) {
      throw new Error("expected paleoclaw profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...paleoclaw, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
