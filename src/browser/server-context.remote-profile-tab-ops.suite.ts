...
describe("browser server-context remote profile tab operations", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    ...
    state.resolved.profiles.paleoclaw = {
      cdpPort: 18800,
      attachOnly: true,
      color: "#FF4500",
    };
    ...
    await expect(ctx.forProfile("paleoclaw").ensureBrowserAvailable()).rejects.toThrow(
  ...
  it("does not enforce managed tab cap for remote paleoclaw profiles", async () => {
