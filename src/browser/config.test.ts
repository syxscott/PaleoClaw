...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("paleoclaw");
    expect(profile?.driver).toBe("paleoclaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const paleoclaw = resolveProfile(resolved, "paleoclaw");
    expect(paleoclaw?.driver).toBe("paleoclaw");
    expect(paleoclaw?.cdpPort).toBe(18800);
    expect(paleoclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from OPENCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ OPENCLAW_GATEWAY_PORT: "19001" }, () => {
      ...
      const paleoclaw = resolveProfile(resolved, "paleoclaw");
      expect(paleoclaw?.cdpPort).toBe(19012);
      expect(paleoclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ OPENCLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const paleoclaw = resolveProfile(resolved, "paleoclaw");
      expect(paleoclaw?.cdpPort).toBe(19022);
      expect(paleoclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(paleoclaw?.cdpPort).toBe(19000);
    expect(paleoclaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "paleoclaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        paleoclaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("paleoclaw");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to paleoclaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps paleoclaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("paleoclaw");
    ...
    it("keeps paleoclaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("paleoclaw");
    ...
    it("keeps paleoclaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
