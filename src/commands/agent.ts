...
function prependInternalEventContext(
  ...
  if (body.includes("paleoclaw runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("paleoclaw agents list")}" to see configured agents.`,
      );
    }
  }
