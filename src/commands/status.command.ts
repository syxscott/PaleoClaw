...
export async function statusCommand(
  ...
  runtime.log(theme.heading("paleoclaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`paleoclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("paleoclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("paleoclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("paleoclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("paleoclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.paleoclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.paleoclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("paleoclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("paleoclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("paleoclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("paleoclaw gateway probe")}`);
  }
}
