...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPENCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("paleoclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-oauth-sync-"));
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-oauth-nosync-"));
    process.env.OPENCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPENCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-oauth-external-"));
    process.env.OPENCLAW_STATE_DIR = tempStateDir;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("paleoclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
