...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("paleoclaw models auth setup-token")}\` or \`${formatCliCommand("paleoclaw configure")}\`.`
          : `Run \`${formatCliCommand("paleoclaw configure")}\` or set an API key env var.`;
