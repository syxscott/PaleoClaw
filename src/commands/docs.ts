...
const SEARCH_TOOL = "https://docs.paleoclaw.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.paleoclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('paleoclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.paleoclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('paleoclaw docs "your query"')}`);
    }
    return;
  }
