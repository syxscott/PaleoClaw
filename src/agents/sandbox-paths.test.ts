...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside paleoclaw tmp root",
      media: path.join(os.tmpdir(), "outside-paleoclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked paleoclaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked paleoclaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked paleoclaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenClawTmp(
