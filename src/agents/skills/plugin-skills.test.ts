    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "paleoclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "paleoclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "paleoclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("paleoclaw-");
  const acpxRoot = await tempDirs.make("paleoclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("paleoclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("paleoclaw-");
  const pluginRoot = await tempDirs.make("paleoclaw-plugin-");
  const outsideDir = await tempDirs.make("paleoclaw-outside-");
