...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("paleoclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("paleoclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("paleoclaw-sbx-");
