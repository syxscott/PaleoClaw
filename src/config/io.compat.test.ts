...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  ...
  dirname: ".paleoclaw",
  port: number,
  filename: string = "paleoclaw.json",
) {
...
describe("config io paths", () => {
  it("uses ~/.paleoclaw/paleoclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".paleoclaw", 19001);
  ...
  it("defaults to ~/.paleoclaw/paleoclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".paleoclaw", "paleoclaw.json"));
    });
  });
  ...
  it("uses OPENCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      ...
      expect(io.configPath).toBe(path.join(home, "svc-home", ".paleoclaw", "paleoclaw.json"));
    });
  });

  it("honors explicit OPENCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".paleoclaw", 20002, "custom.json");
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".paleoclaw", 20003, "legacy-custom.json");
  ...
  it("normalizes safe-bin config entries at config load time", async () => {
    await withTempHome(async (home) => {
      ...
      const configPath = path.join(configDir, "paleoclaw.json");
      await fs.writeFile(
        configPath,
        JSON.stringify(
          {
