...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("paleoclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "paleoclaw-sandbox-browser-global" },
      agentBrowser: { network: "paleoclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("paleoclaw-sandbox-browser-agent");
  });
