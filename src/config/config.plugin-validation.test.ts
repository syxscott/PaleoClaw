...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "paleoclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paleoclaw-config-plugin-validation-"));
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(suiteHome, ".paleoclaw");
