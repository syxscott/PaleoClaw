...
`paleoclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
paleoclaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `paleoclaw acp` and speaks ACP messages over stdio.
...
`paleoclaw acp` resolves the Gateway URL and auth from CLI flags or config:
